...
public class LoginAccountant extends JFrame implements ActionListener {
    ...
    LoginAccountant() {
        ...
        label1 = new JLabel("ACCOUNTANT LOGIN BENCHWORK");
        label1.setForeground(Color.black);
        label1.setFont(new Font("AvantGarde", Font.BOLD, 50));
        label1.setBounds(330, 50, 1000, 200);
        add(label1);
    ...
    public void actionPerformed(ActionEvent e) {
        String Auser_name = textName.getText();
        String Apass = textPName.getText();
        ...
        if (e.getSource() == button1) {
            ...
            if (textName.getText().isEmpty() || textPName.getText().isEmpty()) {
                ...
                try {
                    ...
                    String login_query = "Select * from Accountant Where Auser_name = ? AND Apass = ?";
                    PreparedStatement prst = c1.con.prepareStatement(login_query);
                    prst.setString(1, Auser_name);
                    prst.setString(2, Apass);

                    ResultSet rs = prst.executeQuery();
                    if (rs.next()) {
                        c1.con.close(); // Close connection
                        JOptionPane.showMessageDialog(null, "Accountant login successfully.");
